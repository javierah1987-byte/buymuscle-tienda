...
export default function SideCart({open,onClose}){
  ...
  return(<>
    {open&&<div onClick={onClose} aria-hidden="true" style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:1000}}/>}
    <div role="dialog" aria-modal="true" aria-label="Carrito de compra" aria-hidden={!open} style={{position:'fixed',top:0,right:0,height:'100vh',width:380,maxWidth:'90vw',background:'white',zIndex:1001,boxShadow:'-4px 0 20px rgba(0,0,0,0.15)',transform:open?'translateX(0)':'translateX(100%)',transition:'transform 0.3s ease',display:'flex',flexDirection:'column'}}>
      <div style={{padding:'16px 20px',borderBottom:'1px solid #f0f0f0',display:'flex',justifyContent:'space-between',alignItems:'center',background:'#111'}}>
        ...
        <button onClick={onClose} aria-label="Cerrar carrito" style={{background:'none',border:'none',color:'white',fontSize:22,cursor:'pointer',lineHeight:1}}>✕</button>
      </div>
            ...
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <div style={{display:'flex',border:'1px solid #ddd'}}>
                  <button onClick={()=>updateQty(i.id,i.variant,i.qty-1)} aria-label={'Disminuir cantidad de '+i.name} style={{border:'none',background:'none',width:28,height:28,cursor:'pointer',fontSize:16}}>−</button>
                  <span style={{width:28,textAlign:'center',fontSize:13,fontWeight:600,lineHeight:'28px'}}>{i.qty}</span>
                  <button onClick={()=>updateQty(i.id,i.variant,i.qty+1)} aria-label={'Aumentar cantidad de '+i.name} style={{border:'none',background:'none',width:28,height:28,cursor:'pointer',fontSize:16}}>+</button>
                </div>
                <span style={{fontSize:14,fontWeight:700,color:'#ff1e41'}}>{(i.price*i.qty).toFixed(2)} €</span>
              </div>
            </div>
            <button onClick={()=>remove(i.id,i.variant)} aria-label={'Eliminar '+i.name+' del carrito'} style={{background:'none',border:'none',color:'#ccc',cursor:'pointer',fontSize:18,padding:0,flexShrink:0}}>🗑</button>
          </div>
        ))}
      </div>
      {items.length>0&&(
