...
export default function ProductCard({ product, discountPct: forcedDiscount }: { product: Product; discountPct?: number }) {
  ...
  return (
    <div
      style={{background:'white', border:'1px solid #e8e8e8', position:'relative', transition:'box-shadow 0.2s', boxShadow: hovered ? '0 2px 12px rgba(0,0,0,0.12)' : 'none'}}
    ...
    >
      ...
      <div style={{position:'absolute', top:8, left:8, zIndex:2, display:'flex', flexDirection:'column', gap:4}}>
        {product.stock === 0 && (
          <span style={{background:'#ff2958', color:'white', fontSize:11, fontWeight:700, padding:'2px 8px', textTransform:'uppercase'}}>Agotado</span>
        )}
        {discountPct > 0 && (
      ...
      {/* Imagen */}
      <Link href={`/producto/${product.id}`} style={{display:'block', textDecoration:'none'}}>
      ...
      <div style={{padding:'0.75rem', borderTop:'1px solid #f0f0f0'}}>
        ...
        <Link href={`/producto/${product.id}`} style={{fontSize:14, fontWeight:600, color:'#111', lineHeight:1.3, display:'block', overflow:'hidden', textDecoration:'none', marginBottom:'0.5rem', WebkitLineClamp:2, WebkitBoxOrient:'vertical' as const}}>
        ...
        {/* Botón — outline rojo, al hacer hover se rellena de rojo */}
        <button
          onClick={handleAdd}
          ...
          style={{
            ...
            background: (added || btnHovered) ? 'var(--red)' : 'white',
            border:'1px solid var(--red)',
            color: (added || btnHovered) ? 'white' : 'var(--red)',
