...
export default function AdminClientes(){
  ...
  const TH={padding:'10px 14px',textAlign:'left',fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'0.08em',color:'#888888',borderBottom:'1px solid #e6e6e6'}
  const TD={padding:'12px 14px',fontSize:13,color:'#111',borderBottom:'1px solid #f3f3f3'}
    ...
    <div style={{background:'#f4f5f7',minHeight:'100vh',fontFamily:'Arial,sans-serif',color:'#111'}}>
      <div style={{background:'#ffffff',padding:'18px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid #eaeaea'}}>
        <div><h1 style={{margin:0,fontSize:18,fontWeight:900,textTransform:'uppercase'}}>👥 Clientes</h1>
          <p style={{margin:'3px 0 0',fontSize:12,color:'#888888'}}>{clientes.length} clientes registrados</p>
        </div>
        <div style={{display:'flex',gap:12,alignItems:'center'}}>
          <input value={q} onChange={e=>setQ(e.target.value)} placeholder="Buscar por nombre, email o teléfono..."
            style={{padding:'8px 14px',background:'#eaeaea',border:'1px solid #e0e0e0',color:'#111',fontSize:13,fontFamily:'Arial',width:280}}/>
      ...
      <div style={{display:'grid',gridTemplateColumns:sel?'1fr 380px':'1fr',gap:0,height:'calc(100vh - 70px)'}}>
        <div style={{overflow:'auto'}}>
          {loading?<div style={{padding:40,textAlign:'center',color:'#888888'}}>Cargando...</div>:
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>{filtered.map((c,i)=>(
              <tr key={i} onClick={()=>verDetalle(c)} style={{cursor:'pointer',background:sel?.email===c.email?'rgba(255,30,65,0.08)':'transparent'}}>
                <td style={TD}>
                  <div style={{fontWeight:700,color:c.name?'white':'#888888'}}>{c.name||'Sin nombre'}</div>
                  {c.phone&&<div style={{fontSize:11,color:'#888888',marginTop:2}}>{c.phone}</div>}
                </td>
                <td style={{...TD,color:'#5a5a5a',fontFamily:'monospace',fontSize:12}}>{c.email}</td>
                ...
                <td style={{...TD,color:'#6a6a6a',fontSize:12}}>{c.last?fmt(c.last):'-'}</td>
                <td style={TD}><button onClick={e=>{e.stopPropagation();verDetalle(c)}} style={{background:'transparent',border:'1px solid #d5d5d5',color:'#5a5a5a',padding:'4px 10px',fontSize:11,cursor:'pointer',fontFamily:'Arial'}}>Ver</button></td>
              </tr>
            ))}</tbody>
          </table>}
        </div>
        {sel&&(
          <div style={{borderLeft:'1px solid #e6e6e6',background:'#ffffff',overflow:'auto',padding:20}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <h3 style={{margin:0,fontSize:15,fontWeight:700}}>{sel.name||'Cliente'}</h3>
              <button onClick={()=>setSel(null)} style={{background:'transparent',border:'none',color:'#888888',fontSize:18,cursor:'pointer'}}>×</button>
            </div>
            <div style={{fontSize:12,color:'#6a6a6a',marginBottom:16,lineHeight:1.8}}>
              📧 {sel.email}<br/>
              {sel.phone&&<>📞 {sel.phone}<br/></>}
              🛒 {sel.orders} pedidos · 💰 {sel.total.toFixed(2)} €
            </div>
            <div style={{marginBottom:12,fontWeight:700,fontSize:12,textTransform:'uppercase',letterSpacing:'0.08em',color:'#888888'}}>Historial de pedidos</div>
            {orders.length===0?<div style={{color:'#9a9a9a',fontSize:13}}>Cargando...</div>
            :orders.map((o,i)=>(
              <div key={i} style={{background:'#ffffff',border:'1px solid #eaeaea',padding:'10px 12px',marginBottom:6,borderRadius:4}}>
                ...
                <div style={{fontSize:11,color:'#888888',display:'flex',justifyContent:'space-between'}}>
