    ...
    <div style={{background:'#f5f5f6',minHeight:'100vh',fontFamily:'Arial,sans-serif',color:'#111'}}>
      <div style={{background:'#ffffff',padding:'18px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid #eaeaea'}}>
        <div>
          ...
          <p style={{margin:'3px 0 0',fontSize:12,color:'#888888'}}>{carts.length} carritos sin completar</p>
        </div>
      ...
      <div style={{padding:'20px 28px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:20}}>
          {[
            ...
            <div key={k.label} style={{background:'#ffffff',border:'1px solid #eaeaea',padding:16,textAlign:'center'}}>
              <div style={{fontSize:24,marginBottom:4}}>{k.icon}</div>
              <div style={{fontSize:22,fontWeight:900,color:'#ff1e41'}}>{k.val}</div>
              <div style={{fontSize:10,color:'#888888',textTransform:'uppercase',letterSpacing:'0.08em',marginTop:2}}>{k.label}</div>
            </div>
          ))}
        </div>
        {loading?<div style={{textAlign:'center',padding:40,color:'#888888'}}>Cargando...</div>
        :carts.length===0?<div style={{textAlign:'center',padding:40,color:'#9a9a9a',fontSize:14}}>No hay carritos abandonados registrados todavia.</div>
        :<table style={{width:'100%',borderCollapse:'collapse'}}>
          <thead>
            <tr style={{borderBottom:'1px solid #e0e0e0'}}>
              {['Email','Productos','Valor','Fecha','Estado','Accion'].map(h=>(
                <th key={h} style={{padding:'9px 12px',textAlign:'left',fontSize:10,fontWeight:700,textTransform:'uppercase',color:'#888888',letterSpacing:'0.08em'}}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {carts.map(c=>(
              <tr key={c.id} style={{borderBottom:'1px solid #f3f3f3'}}>
                <td style={{padding:'10px 12px',fontSize:12,color:'#5a5a5a'}}>{c.email||'—'}</td>
                <td style={{padding:'10px 12px',fontSize:13,color:'#4a4a4a'}}>{count(c)} producto{count(c)!==1?'s':''}</td>
                <td style={{padding:'10px 12px',fontSize:13,fontWeight:700,color:'#ff1e41'}}>{total(c).toFixed(2)} €</td>
                <td style={{padding:'10px 12px',fontSize:12,color:'#6a6a6a'}}>{c.created_at?fmt(c.created_at):'—'}</td>
                ...
                <td style={{padding:'10px 12px'}}>
                  {!c.recovery_email_sent&&c.email&&(
                    <button onClick={()=>sendRecovery(c)} disabled={sending===c.id}
                      style={{padding:'5px 12px',background:'#ff1e41',border:'none',color:'#111',fontSize:12,fontWeight:700,cursor:'pointer',fontFamily:'inherit'}}>
