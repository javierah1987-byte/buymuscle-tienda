...
export default function AdminResenas(){
  ...
  const Stars=({n})=>(
    <span style={{letterSpacing:2,fontSize:14}}>
      {[1,2,3,4,5].map(i=><span key={i} style={{color:i<=n?'#f59e0b':'#d5d5d5'}}>★</span>)}
    </span>
  )
    ...
    <div style={{background:'#f4f5f7',minHeight:'100vh',fontFamily:'Arial,sans-serif',color:'#111'}}>
      <div style={{background:'#ffffff',padding:'18px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',borderBottom:'1px solid #eaeaea'}}>
        <div>
          ...
          <p style={{margin:'3px 0 0',fontSize:12,color:'#888888'}}>Modera las reseñas antes de publicarlas</p>
        </div>
      ...
      <div style={{padding:'16px 28px 0',display:'flex',gap:4}}>
        {[{k:'pending',l:'Pendientes de aprobar'},{k:'approved',l:'Aprobadas y publicadas'}].map(t=>(
          <button key={t.k} onClick={()=>setTab(t.k)}
            style={{padding:'8px 18px',background:tab===t.k?'rgba(255,30,65,0.15)':'transparent',border:'1px solid',borderColor:tab===t.k?'rgba(255,30,65,0.4)':'#dcdcdc',color:'#111',fontSize:13,cursor:'pointer',fontFamily:'Arial',borderRadius:'4px 4px 0 0'}}>
            {t.l}
          </button>
        ))}
      </div>
      ...
      <div style={{padding:'20px 28px'}}>
        {loading
          ?<div style={{textAlign:'center',padding:40,color:'#888888'}}>Cargando...</div>
          :reviews.length===0
          ?<div style={{textAlign:'center',padding:40,color:'#9a9a9a',fontSize:14}}>
            No hay reseñas {tab==='pending'?'pendientes de aprobar':'aprobadas'}.<br/>
            {tab==='pending'&&<span style={{fontSize:12,color:'#cccccc'}}>Las reseñas de clientes aparecerán aquí para que las moderes.</span>}
          </div>
          :reviews.map((r)=>(
            <div key={r.id} style={{background:'#ffffff',border:'1px solid #e8e8e8',padding:20,marginBottom:12,borderRadius:4}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:6,flexWrap:'wrap'}}>
                    <Stars n={r.rating||0}/>
                    <span style={{fontSize:13,fontWeight:700,color:'#111'}}>{r.name||'Anónimo'}</span>
                    {r.email&&<span style={{fontSize:11,color:'#888888',fontFamily:'monospace'}}>{r.email}</span>}
                  </div>
                  ...
                  {r.comment&&<p style={{margin:0,fontSize:14,color:'#444444',lineHeight:1.6,maxWidth:600,fontStyle:'italic'}}>"{r.comment}"</p>}
                </div>
