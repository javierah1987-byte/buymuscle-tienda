...
async function getByCategory(cat: string, limit = 4) {
  ...
  const { data } = await supabase.from('products').select('*, categories(name)')
    .eq('active', true).eq('category_id', catData.id).gt('stock', 0).limit(limit)
  return data || []
}
...
export default async function Home() {
  const [featured, proteinas, creatinas] = await Promise.all([
    getFeatured(),
    getByCategory('Proteínas', 4),
    getByCategory('Creatinas Monohidratos', 4),
  ])
        ...
        <div className="container">
          <div className="hero-content" style={{ maxWidth: 680 }}>
            ...
            <h1 className="hero-title">
              SUPLEMENTACIÓN<br />
              DE <span className="accent">ÉLITE</span>
            </h1>
            <p className="hero-subtitle">
              Más de 300 productos de las mejores marcas mundiales. Proteínas, creatinas, pre-entrenos y mucho más.
            </p>
            <div className="hero-actions">
              <Link href="/tienda" className="btn-primary" style={{ fontSize: 15, padding: '13px 32px' }}>
                Ver catálogo completo
              </Link>
              <Link href="/tienda?cat=Lo%20M%C3%A1s%20Vendido" className="btn-outline" style={{ fontSize: 15, padding: '11px 28px' }}>
                Lo más vendido
              </Link>
            </div>
        ...
        <div className="container">
          <div style={{ display: 'flex', gap: '0.75rem', overflowX: 'auto', scrollbarWidth: 'none' }}>
            {[
              ...
              { name: 'Creatinas Monohidratos', emoji: '⚡' },
              { name: 'Pre-entrenos', emoji: '🔥' },
              { name: 'Vitaminas', emoji: '💊' },
              { name: 'BCAA', emoji: '💪' },
              { name: 'Quemadores', emoji: '🎯' },
              { name: 'Snacks Protéicos', emoji: '🍫' },
              { name: 'Accesorios', emoji: '🎒' },
            ].map(c => (
              <Link key={c.name} href={`/tienda?cat=${encodeURIComponent(c.name)}`}
                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', padding: '10px 18px', display: 'flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap', transition: 'all 0.15s', flexShrink: 0, color: 'rgba(255,255,255,0.8)', fontSize: 13, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.03em' }}
                ...
                onMouseLeave={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(255,255,255,0.05)'; (e.currentTarget as HTMLAnchorElement).style.color = 'rgba(255,255,255,0.8)'; (e.currentTarget as HTMLAnchorElement).style.borderColor = 'rgba(255,255,255,0.08)'; }}>
      ...
      {creatinas.length > 0 && (
        <section style={{ padding: '3rem 0', background: 'var(--bg)' }}>
          <div className="container">
            ...
            <div className="products-grid">
              {creatinas.map((p: any) => <ProductCard key={p.id} product={p} />)}
            </div>
          </div>
        </section>
      )}
        ...
        <div className="container">
          <div className="dist-banner">
            <div>
              ...
              <p style={{ color: 'rgba(255,255,255,0.5)', maxWidth: 480, lineHeight: 1.6, fontSize: 14 }}>
                Accede con tus credenciales para ver precios exclusivos según tu nivel. Hasta un 20% de descuento.
              </p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <Link href="/distribuidores/login" className="btn-primary" style={{ fontSize: 14, padding: '13px 28px', justifyContent: 'center' }}>
                Acceder al portal
              </Link>
              <Link href="/distribuidores" className="btn-outline" style={{ fontSize: 13, padding: '10px 24px', justifyContent: 'center' }}>
                Más información
              </Link>
            </div>
          </div>
