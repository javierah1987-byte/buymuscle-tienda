...
const CAT_GROUPS = [
  { label:'Nutrición Deportiva', cats:['Proteínas','Proteína Whey','Proteína Isolatada','Proteína Vegetal','Caseínas','Ganadores de Peso','Barritas Protéicas','Snacks Protéicos','Creatinas Monohidratos','Pre-entrenos','Recuperadores','BCAA','Aminoácidos','Glutaminas','L-Carnitina','Termogénicos'] },
  { label:'Vitaminas y Salud', cats:['Vitaminas','Minerales','Omega 3','Colágeno','Sistema Inmunológico'] },
  { label:'Objetivos', cats:['Ganar Músculo','Control de Peso','Quemadores','Rendimiento Deportivo'] },
          ...
          <aside style={{ background: 'white', border: '1px solid #e8e8e8', position: 'sticky', top: 8 }}>
            {CAT_GROUPS.map((group, gi) => (
              <div key={gi} style={{ borderBottom: '1px solid #e8e8e8' }}>
                <button onClick={() => toggleGroup(gi)} style={{ width: '100%', padding: '10px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.06em', color: '#111', background: 'none', border: 'none', cursor: 'pointer' }}>
                  {group.label}
                  <span style={{ fontSize: 9, color: '#aaa' }}>{isGroupOpen(gi) ? '▼' : '▶'}</span>
                </button>
                {isGroupOpen(gi) && (
            ...
            <SidebarSection title="Precio">
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#666', marginTop: 4 }}>
                <span>1,00 €</span><span style={{ fontWeight: 600 }}>{maxPrice},00 €</span>
              </div>
            </SidebarSection>
          ...
          <div>
            <div style={{ background: 'white', border: '1px solid #e8e8e8', padding: '14px 16px', marginBottom: '1px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' as const, gap: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' as const }}>
                {(cat || q) && <Link href="/tienda" style={{ fontSize: 12, color: '#999', textDecoration: 'none', border: '1px solid #e8e8e8', padding: '5px 10px' }}>✕ Quitar filtro</Link>}
            ...
            {!cat && !q && (
              <div style={{ background: 'white', border: '1px solid #e8e8e8', borderTop: 'none', padding: '10px 14px', marginBottom: '1px', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' as const }}>
                {['Proteínas','Creatinas Monohidratos','Pre-entrenos','BCAA','Vitaminas','Quemadores','Snacks Protéicos','Veganos'].map(c => (
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: '0.25rem', marginTop: '1.5rem', flexWrap: 'wrap' as const }}>
                {page > 1 && <Link href={`/tienda?cat=${encodeURIComponent(cat)}&q=${encodeURIComponent(q)}&page=${page - 1}`} style={{ padding: '8px 14px', fontSize: 13, border: '1px solid #ddd', background: 'white', color: '#333', textDecoration: 'none' }}>← Anterior</Link>}
                ...
                {page < totalPages && <Link href={`/tienda?cat=${encodeURIComponent(cat)}&q=${encodeURIComponent(q)}&page=${page + 1}`} style={{ padding: '8px 14px', fontSize: 13, border: '1px solid #ddd', background: 'white', color: '#333', textDecoration: 'none' }}>Siguiente →</Link>}
              </div>
            )}
          </div>
        </div>
