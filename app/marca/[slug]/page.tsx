...
export default function MarcaPage(){
  ...
  useEffect(()=>{
    if(!slug)return
    setLoading(true);setNotFound(false);setBrand(null);setProducts([])
    fetch(S+'/rest/v1/brands?slug=eq.'+encodeURIComponent(slug),{headers:h})
      .then(r=>r.json())
      .then(d=>{
        const b=Array.isArray(d)&&d.length>0?d[0]:null
    ...
    <div style={{fontFamily:'Arial,sans-serif'}}>
      <div style={{background:'linear-gradient(135deg,#000 0%,#1a1a1a 50%,#000 100%)',padding:'60px 32px',textAlign:'center',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'relative'}}>
          ...
          {loading&&<div style={{marginTop:24,fontSize:14,color:'rgba(255,255,255,0.4)'}}>Cargando...</div>}
        </div>
      </div>
